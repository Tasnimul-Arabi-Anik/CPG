...
def render_packet(root: Path, row: dict[str, str], template: dict[str, str]) -> str:
    ...
    if evidence_id in {"rbp_domain_evidence", "rbp_structural_evidence"}:
        protein_handoff_lines = [
            ...
            "",
            "```bash",
            "python scripts/normalize_rbp_external_evidence.py --domain-input <reviewed_hmmer.domtblout> --domain-format hmmer_domtblout --hmmer-mode hmmsearch --annotation-manifest results/annotations/phage_annotations.tsv --database <reviewed_profile_database> --database-version <reviewed_snapshot> --domain-output data/metadata/external_evidence/rbp_domain_evidence.tsv --structural-output data/metadata/external_evidence/rbp_structural_evidence.tsv --report-output results/qc/rbp_external_evidence_normalization_report.tsv",
            "```",
            "",
            "For reviewed generic domain and Foldseek/Phold-style structural tables:",
            "",
            "```bash",
            "python scripts/normalize_rbp_external_evidence.py --domain-input <reviewed_domain_hits.tsv> --domain-format generic_tsv --structural-input <reviewed_structural_hits.tsv> --structural-format foldseek_tsv --foldseek-fields query,target,alntmscore,prob,evalue --annotation-manifest results/annotations/phage_annotations.tsv --database <reviewed_profile_or_structure_database> --database-version <reviewed_snapshot> --domain-output data/metadata/external_evidence/rbp_domain_evidence.tsv --structural-output data/metadata/external_evidence/rbp_structural_evidence.tsv --report-output results/qc/rbp_external_evidence_normalization_report.tsv",
            "```",
            "",
            "After review, configure the populated target TSVs in `config/workflow.yaml` as `inputs.domain_evidence` and `inputs.structural_evidence`. Header-only normalizer outputs are not accepted evidence. Existing evidence outputs are preserved when an input is absent unless `--overwrite-empty` is explicitly supplied.",
            "",

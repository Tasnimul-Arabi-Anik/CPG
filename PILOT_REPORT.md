...
Prodigal, BLASTN, Mash, and fastANI are available and have been used for current production evidence or robustness baselines. Pharokka `1.9.1` was installed in the `pharokka` conda environment for the 30-phage pilot. Phold `1.2.5` and Foldseek `10.941cd33` were installed in the `phold` conda environment for the structural pilot. Reviewed Kaptive/Kleborate outputs are available, but their executables are not currently on `PATH`. HMMER, DefenseFinder, and PADLOC are not currently available on `PATH`; they should be run directly once installed rather than replaced with custom implementations.
...
- Preliminary genome-similarity clustering/model baselines using current BLASTN, Mash, and fastANI evidence, with clear limitations.
...
- The BLASTN, Mash, or fastANI baselines replace VIRIDIC/skani or equivalent manuscript-grade phage clustering.

## Recommended Next Pilot Step

The 30-phage pilot justified full-set annotation scaling, and both Pharokka and Phold/Foldseek have now completed across all 105 assay phages. A pairwise receptor-layer feature matrix and grouped H1 rate-baseline comparisons have also been generated with BLASTN, Mash, and fastANI baselines. The next bounded action is either a VIRIDIC/skani-style phage intergenomic similarity run or reviewed RBP domain evidence, depending on tool/database availability. Do not claim receptor specificity or model superiority from annotation coverage alone.
...
Claim boundary: this is an initial quantitative H1 test on spot-test interaction outcomes only. It is not evidence of productive infection and does not address defense/counter-defense compatibility. In the current pilot, receptor-feature summaries do not yet outperform the BLASTN nearest-phage plus host K/O baseline under cold-phage, cold-K-locus, or cold-cluster splits. Fold-level intervals and sign-flip checks are diagnostic only. The held-out-group bootstrap intervals are benchmark-specific and use the current grouped folds, not an independent external validation. Treat any apparent model advantage as provisional until independent validation, leakage checks, and VIRIDIC/skani-style manuscript-grade phage taxonomy/similarity baselines are added. A fastANI robustness run produced 5,460 assay-phage pair rows with 727 ANI hits; receptor summaries still did not outperform fastANI nearest-phage plus host K/O under cold-cluster or cold-K-locus evaluation.

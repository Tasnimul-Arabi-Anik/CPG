...
Reviewed host-by-phage matrices are normalized before import with `scripts/normalize_assay_matrix.py` and `config/assay_matrix_sources.yaml`. The PhageHostLearn 2024 Zenodo matrix is configured as a disabled source until the CSV is downloaded locally and its phage and host source IDs are mapped to canonical local entities. Matrix blanks are treated as untested cells, explicit `1/0` values become spot-test positive or tested-negative rows, and unresolved entity mappings are reported and skipped rather than silently entering the assay table. `scripts/create_assay_matrix_mapping_templates.py` extracts matrix source IDs into pending review templates; pending mappings are ignored until a reviewer marks them reviewed. `scripts/create_phagehostlearn_source_exports.py` creates disabled benchmark phage and host source exports with deterministic canonical IDs so those pending map rows can be reviewed against source entities before any assay rows are enabled. `scripts/audit_phagehostlearn_readiness.py` reports whether those benchmark artifacts are still blocked by pending maps, disabled sources, missing host evidence, or absent assay rows. The workflow runner exposes `stage_0_assay_matrix_normalization`, but `assay_matrix_normalization.enabled` remains false by default so seed and clean-checkout runs cannot import unreviewed benchmark outcomes.

`data/metadata/external/phagehostlearn/phagehostlearn_file_manifest.tsv` records the expected local Zenodo files used by the benchmark path, including direct retrieval URLs, advisory commands, byte counts, MD5 values, and SHA-256 values when local review has been completed. `scripts/validate_phagehostlearn_external_files.py` checks local copies without downloading them and writes `results/<profile>/qc/phagehostlearn_2024_external_files.tsv`. Missing files are warnings for seed/mock runs, while present files with size or source-MD5 mismatches are blocking; SHA-256 can remain pending for large files until local review is performed. Schema details are in `docs/phagehostlearn_external_file_manifest_schema.md`.

`scripts/audit_phagehostlearn_host_archive.py` compares host source IDs in the PhageHostLearn host export with FASTA members in the verified host-genome archive and writes `results/<profile>/qc/phagehostlearn_2024_host_archive.tsv`. This supports manual host entity review only; it does not approve maps, infer K/O/ST calls, or import assay rows. Schema details are in `docs/phagehostlearn_host_archive_schema.md`.

`scripts/audit_phagehostlearn_metadata_support.py` adds a curation-only support audit for the same benchmark. It compares matrix phage IDs against `RBPbase.csv` and matrix host IDs against `Locibase.json`/`Locibase_invitro.json`, while also reporting source-export and source-to-canonical map review status. Missing local RBPbase or Locibase files are warnings so seed workflows remain network-free; malformed provided files are blocking. The output is `results/<profile>/qc/phagehostlearn_2024_metadata_support.tsv` with a companion report. This audit helps reviewers decide which benchmark entities have receptor-layer support metadata, but it does not approve mappings, import assay rows, or establish receptor specificity/productive infection claims. Schema details are in `docs/phagehostlearn_metadata_support_schema.md`.

`scripts/audit_phagehostlearn_map_review.py` consumes the metadata-support output plus the benchmark source exports and source-to-canonical maps. It writes `results/<profile>/qc/phagehostlearn_2024_map_review.tsv`, reporting whether each map row is still blocked by pending source-entity review, waiting for local matrix evidence, structurally ready for manual review, or already usable by the matrix normalizer. The audit never edits map files and never promotes pending rows; it is a review checklist for the manual approval step required before canonical assay rows are emitted. Schema details are in `docs/phagehostlearn_map_review_schema.md`.

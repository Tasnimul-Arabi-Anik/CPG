...
After external-evidence planning, `scripts/create_production_evidence_handoff.py` writes `results/qc/production_evidence_handoff.md`. This generated handoff distinguishes current bridge evidence from production-grade external evidence expected for manuscript claims, including Pharokka/PHROGs annotation, VIRIDIC/Mash/skani-style similarity, RBP domain/structural evidence, Kleborate/Kaptive host features, and DefenseFinder/PADLOC host-defense calls.
...
A fastANI robustness baseline is also available for the 105 assay phages. It is generated by `scripts/build_fastani_pairwise_similarity.py`, which runs fastANI and normalizes the output into the same pairwise schema. Missing fastANI hits are retained as explicit zero rows for complete pairwise coverage; this means no fastANI-reported ANI hit, not proof of no homology. Regeneration command:

```bash
python scripts/build_fastani_pairwise_similarity.py \
  --threads 64 \
  --output data/metadata/production_evidence/phage_fastani_similarity.tsv \
  --report-output data/metadata/production_evidence/phage_fastani_similarity_report.tsv
```

The current fastANI table has 5,460 assay-phage pairs, 727 reported ANI hits, and 4,733 explicit no-hit rows. fastANI is a useful ANI robustness baseline but remains distinct from VIRIDIC-style phage intergenomic similarity.

A skani robustness baseline is also available for the 105 assay phages. It is generated by `scripts/build_skani_pairwise_similarity.py`, which runs `skani triangle --small-genomes --full-matrix` and normalizes ANI plus minimum bidirectional aligned fraction into the same pairwise schema. Regeneration command used locally:
...
The current skani table has 5,460 assay-phage pairs and 709 pairs with nonzero ANI/AF support. `coverage_percent` is the minimum bidirectional aligned fraction from skani's AF matrix, not VIRIDIC intergenomic similarity coverage. The repository script defaults to `skani`; the local command above records the temporary environment used for this run.
...
For the PhageHostLearn assay-phage production profile, an external PHROGs profile search was run with MMseqs2 18.8cc5c against the local Pharokka PHROGs profile database. `scripts/build_phrogs_profile_domain_evidence.py` is glue code only: it joins the MMseqs tabular output to `phrog_annot_v4.tsv`, keeps receptor-relevant PHROG annotations, applies a minimum query coverage threshold, and writes the generic domain-evidence input consumed by `scripts/normalize_rbp_external_evidence.py`. The current reviewed input has 495 PHROGs receptor-domain rows across 242 proteins and 93 PHROGs. This is computational domain-profile candidate evidence, not proof of receptor specificity or depolymerase activity.
...
Optional domain and structural evidence schemas are documented in `docs/rbp_depolymerase_schema.md`. For the PhageHostLearn production profile, `scripts/build_phrogs_profile_domain_evidence.py` converts reviewed MMseqs2 PHROGs profile-search hits into `data/metadata/production_evidence/phrogs_profile_receptor_domain_input.tsv`, and `scripts/normalize_rbp_external_evidence.py` normalizes those rows into `data/metadata/production_evidence/rbp_domain_evidence.tsv`. The current normalized domain-evidence table has 495 rows across 242 proteins. `scripts/build_phold_foldseek_structural_evidence.py` maps reviewed Phold/Foldseek receptor-like CDS rows to production Prodigal annotation IDs by same-phage coordinate overlap. The current run maps 23 Foldseek receptor-like rows, excludes 213 Pharokka-carried rows from structural evidence, and reports 0 unmapped rows. The normalized domain and structural evidence tables are configured as `inputs.domain_evidence` and `inputs.structural_evidence` in `config/workflow.production.yaml`; both are computational candidate evidence only.
...
For the phage side, `scripts/build_phold_acr_antidefense_evidence.py` normalizes existing Phold ACR subdatabase hits from `results/production/phold_assay_phages/*/sub_db_tophits/acr_cds_predictions.tsv` into `data/metadata/production_evidence/phage_antidefense_candidates.tsv`. It maps hits back to sequence-backed Prodigal CDS rows by phage ID, coordinates, and strand. The current output has 7 accepted computational anti-CRISPR candidates across 7/105 assay phages from 105 inspected Phold ACR files; one raw ACR row did not map cleanly to the reviewed CDS table and is excluded. These rows are accepted counter-defense candidate evidence for coverage auditing, not proof of defense escape or productive infection.
...
Optional PADLOC/DefenseFinder-style and phage anti-defense schemas are documented in `docs/defense_counterdefense_schema.md`. The production profile now supplies DefenseFinder host-defense input and sparse Phold ACR anti-CRISPR input. Annotation-keyword phage hits remain screening-only; H4 remains blocked because productive-infection, plaque, propagation, or EOP outcomes are absent and counter-defense coverage is limited.

## Stage 7: Feature-Set Model Comparison

The model comparison script consumes phage clusters, RBP/depolymerase candidates, phage-host links, compatibility features, optional phage/host bridge metadata, and canonical phage-host assay rows when available. It writes transparent leave-one-out categorical baselines for K/O association proxies, group summaries for prophage RBP reservoirs, host background defense burden, exploratory novelty context, assay-derived spot-test breadth, and assay-feature coverage. It does not emit pair-level H1b spot-interaction model-performance rows from RBPbase/Locibase bridge metadata in this readiness step. H1b pairwise modeling remains blocked until production receptor/RBP features and grouped cold-host/cold-phage/cold-study evaluation are implemented in a later analysis. H3 retains tested-host denominators, positive spot-test counts, continuous spot-positive fractions, and Wilson intervals for the observed tested panel; these are initial-interaction summaries only. H4 remains explicitly blocked until productive-infection, plaque, EOP, or propagation outcomes are curated.
...
Project-specific Phold/Foldseek production evidence is prepared in two steps. First, `scripts/build_phold_foldseek_structural_evidence.py` maps non-Pharokka Foldseek receptor-like Phold rows to production Prodigal `annotation_gene_id` values by coordinate overlap and writes `data/metadata/production_evidence/phold_foldseek_receptor_structural_input.tsv`. Second, `scripts/normalize_rbp_external_evidence.py` validates and normalizes those rows into `data/metadata/production_evidence/rbp_structural_evidence.tsv`. The current production structural evidence has 23 normalized rows, 0 unmapped rows, and coverage for 12/105 assay phages. PHROGs/MMseqs domain evidence is prepared separately from the all-protein FASTA exported by `scripts/export_external_evidence_proteins.py`; the normalized domain table has 495 rows and expands Stage 4 candidate support to 242 domain-supported proteins. These steps standardize computational evidence only; they do not infer receptor specificity.

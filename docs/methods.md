...
The model comparison script consumes phage clusters, RBP/depolymerase candidates, phage-host links, compatibility features, optional phage/host bridge metadata, and canonical phage-host assay rows when available. It writes transparent leave-one-out categorical baselines for K/O association proxies, pair-level H1b spot-interaction screening rows, group summaries for prophage RBP reservoirs, host background defense burden, exploratory novelty context, and assay-derived spot-test breadth. The H1b `spot_interaction_*` rows use explicit tested positive and tested negative spot-test pairs plus optional seed RBPbase/Locibase bridge metadata, but they are screening rows only: they are not grouped/cold-split primary evidence and remain claim-blocked until production receptor/RBP features are available. H3 retains tested-host denominators, positive spot-test counts, continuous spot-positive fractions, and Wilson intervals for the observed tested panel; these are initial-interaction summaries only. H4 remains explicitly blocked until productive-infection, plaque, EOP, or propagation outcomes are curated.

...
| C2 | H1 | RBP/depolymerase modules predict K/O tropism or pairwise receptor compatibility better than taxonomy. | Populated `model_comparison.tsv` rows for K/O association; tested positive and negative rows in `data/metadata/phage_host_assays.tsv` for pairwise prediction. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; `results/validation/phage_host_assay_validation.tsv`; `results/mock/models/model_comparison.tsv` | `data_dependent`; pairwise host-range prediction is blocked until assay outcomes exist. | `RBP/depolymerase features can be quantitatively compared against taxonomy-only predictors once outcome labels are curated.` | `RBP modules outperform taxonomy for host range` unless supported by real populated model metrics and assay labels. |
...
| C4 | H3 | Generalist or broad-host-range phages have more modular RBPs and/or counter-defense genes. | Tested host panels with susceptible-host numerator and tested-host denominator plus RBP modularity and anti-defense burden tests. | `data/metadata/phage_host_assays.tsv`; `results/validation/phage_host_assay_validation.tsv`; future breadth-aware model rows | `data_dependent`; currently blocked because explicit host-range breadth labels are absent. | `The workflow defines the assay data needed to test whether modular RBP and counter-defense burden associate with host-range breadth.` | `Broad host range is caused by modular RBPs` from metadata-only comparisons. |
| C5 | H4 | Receptor plus defense/counter-defense models improve productive-infection prediction among receptor-compatible pairs. | Tested pairwise productive-infection, plaque, or EOP outcomes plus receptor, host-defense, and phage counter-defense features. | `data/metadata/phage_host_assays.tsv`; `results/validation/phage_host_assay_validation.tsv`; future leakage-safe pairwise model rows | `data_dependent`; currently blocked because productive-infection labels are absent. | `The workflow defines the assay and evidence requirements for evaluating a second-filter defense/counter-defense model.` | `The model predicts infection success` from compatibility features or metadata host links alone. |

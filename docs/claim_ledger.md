...
| C2 | H1 | RBP/depolymerase modules predict K/O tropism or pairwise receptor compatibility better than taxonomy. | Populated `model_comparison.tsv` rows for K/O association; tested positive and negative rows in `results/<profile>/metadata/phage_host_assays.tsv` for pairwise prediction. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; `results/validation/phage_host_assay_validation.tsv`; `results/mock/models/model_comparison.tsv` | `data_dependent`; seed spot-test outcomes, production host K/O evidence, production assay-phage baseline CDS evidence, and exact RBPbase ML candidate matches for the PhageHostLearn benchmark exist, but pairwise receptor-compatibility claims remain blocked until domain/structural support where required, leakage-safe grouped model evaluation, and claim-support audits are available. | `RBP/depolymerase features can be quantitatively compared against taxonomy-only predictors once production feature evidence and reviewed outcome labels are jointly available.` | `RBP modules outperform taxonomy for host range` unless supported by real populated model metrics and assay labels. |
...
| C6 | H5 | Clinically important lineages carry distinctive prophage and defense repertoires. | Host ST/AMR/virulence metadata, prophage links, defense annotations, and lineage-level summaries. | `results/host_features/host_metadata.tsv`; `results/defense_systems/host_defense_systems.tsv`; `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv` | `data_dependent`; production ST/AMR/virulence evidence is partial for the PhageHostLearn benchmark, but host-defense/prophage evidence and public sampling bias controls are still required. | `Lineage-level differences are tested as associations.` | `Clinical lineages are intrinsically phage-resistant` without controlled sampling and infection assays. |

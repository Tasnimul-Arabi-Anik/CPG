...
| C2 | H1 | RBP/depolymerase modules predict K/O tropism or pairwise receptor compatibility better than taxonomy. | Populated `model_comparison.tsv` rows for K/O association; tested positive and negative rows in `results/<profile>/metadata/phage_host_assays.tsv` for pairwise prediction. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; `results/validation/phage_host_assay_validation.tsv`; `results/mock/models/model_comparison.tsv` | `data_dependent`; seed spot-test outcomes exist, but pairwise receptor-compatibility claims remain blocked until production RBP/depolymerase evidence, host K/O evidence, and leakage-safe model evaluation are available. | `RBP/depolymerase features can be quantitatively compared against taxonomy-only predictors once production feature evidence and reviewed outcome labels are jointly available.` | `RBP modules outperform taxonomy for host range` unless supported by real populated model metrics and assay labels. |

...
| C2 | H1 | RBP/depolymerase modules predict K/O association better than taxonomy. | Populated `model_comparison.tsv` rows comparing taxonomy, genome similarity, and RBP feature sets for K/O targets. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; `results/mock/models/model_comparison.tsv` | `data_dependent`; mock workflow only verifies the test. | `RBP/depolymerase features can be quantitatively compared against taxonomy-only predictors.` | `RBP modules outperform taxonomy` unless supported by real populated model metrics. |
...
| C4 | H3 | Generalist or broad-host-range phages have more modular RBPs and/or counter-defense genes. | Host-range breadth metadata plus RBP modularity and anti-defense burden tests. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; `results/models/feature_importance.tsv` | `data_dependent`; current scaffold does not include experimental host-range breadth labels. | `The workflow tests whether modular RBP and counter-defense burden associate with host-range breadth.` | `Broad host range is caused by modular RBPs` from metadata-only comparisons. |
| C5 | H4 | Receptor plus defense/counter-defense models improve host compatibility prediction. | Model comparison between receptor-only, defense-only, counter-defense-only, and combined feature sets. | `results/models/model_comparison.tsv`; `results/models/hypothesis_summary.tsv`; Figure 5 source data | `data_dependent`; infectivity labels are needed for true host-range prediction. | `Combined receptor and defense/counter-defense features are evaluated as a second-filter compatibility model.` | `The model predicts infection success` without adsorption/infection assay labels. |

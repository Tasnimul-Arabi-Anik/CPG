...
| Genome similarity | Production BLASTN pairwise baseline across 107 sequence-QC-passing phage-like records; Mash and fastANI robustness baselines across 105 assay phages | BLASTN/Mash/fastANI baselines are useful for reproducible benchmarking but are not final species-like phage clustering methods | VIRIDIC, skani, or documented phage-appropriate all-vs-all comparison across sequence-backed phages/prophages | Baselines available; manuscript-grade phage similarity still planned |
...
3. Supplement the current BLASTN/Mash/fastANI baselines with VIRIDIC, skani, or another documented phage-appropriate species-like similarity method before final taxonomy claims.

...
| Annotation | Sequence-backed CDS annotations for 105 assay phages plus 24 GenBank bridge CDS rows for one computational prophage, receptor-domain evidence, and Phold/Foldseek receptor-like evidence | Benchmark evidence is not a comprehensive public-scale reannotation; the prophage rows are bridge annotation only and currently yield zero RBP/depolymerase candidates | Standardized Pharokka/PHROGs plus reviewed domain/structural evidence across all sequence-backed phage/prophage genomes | Benchmark implemented; prophage atlas-scale analysis pending |
| RBP/depolymerase evidence | 495 PHROGs/MMseqs receptor-domain rows, 23 Phold/Foldseek receptor-like structural rows, 105 phage-level module identity signatures, and ordered per-gene PHROGs/MMseqs architecture proxies for assay phages | Exact unordered module identity signatures improve over RBPbase but do not robustly outperform BLASTN genome-similarity plus K/O; ordered architecture proxies are too sparse under cold-phage/cold-cluster exact-key evaluation; cold-K support diagnostics show asymmetric fallback behavior | Expand/curate receptor evidence and test external/generalization cohorts before claiming superiority | Benchmark implemented; module signal available but claim not supported |

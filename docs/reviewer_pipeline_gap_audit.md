...
| Annotation | Sequence-backed CDS annotations for 105 assay phages plus 24 GenBank bridge CDS rows for one computational prophage, receptor profile-family/structural evidence, and Phold/Foldseek receptor-like evidence | Benchmark evidence is not a comprehensive public-scale reannotation; the prophage rows are bridge annotation only and currently yield zero RBP/depolymerase candidates | Standardized Pharokka/PHROGs plus reviewed domain/structural evidence across all sequence-backed phage/prophage genomes | Benchmark implemented; prophage atlas-scale analysis pending |
| RBP/depolymerase evidence | 495 PHROGs/MMseqs receptor profile-family hit rows, 23 Phold/Foldseek receptor-like structural rows, 105 phage-level profile-family/structural hit signatures, and ordered per-gene PHROGs/MMseqs profile-hit proxies for assay phages | Exact unordered profile-family/structural hit signatures improve over RBPbase but do not robustly outperform BLASTN genome-similarity plus K/O; ordered profile-hit proxies are too sparse under cold-phage/cold-cluster exact-key evaluation; cold-K support diagnostics show asymmetric fallback behavior | Expand/curate receptor evidence and test external/generalization cohorts before claiming superiority | Benchmark implemented; profile-hit signal available but claim not supported |
...
| Defense/counter-defense | 2,758 DefenseFinder host-defense rows and 7 explicit Phold ACR candidates; annotation-keyword hits remain screening-only | No productive-infection/plaque/EOP outcomes exist, so H4 cannot be tested from the current endpoint | Curate productive-infection outcomes before testing whether defense/counter-defense improves prediction | Evidence layer implemented; H4 future work for dry-lab benchmark |
| Statistical modeling | 10,006 spot-test pairs support H1 initial-interaction benchmarking; H3 has descriptive spot breadth; H4 is explicitly future work | Spot tests are not productive infection, and current H1 profile-hit signatures do not robustly beat genome-similarity plus K/O baselines | Leakage-safe grouped analyses with appropriate outcomes, uncertainty, and claim audits after each evidence expansion | H1 benchmark implemented; H4 non-blocking limitation for current dry-lab endpoint |

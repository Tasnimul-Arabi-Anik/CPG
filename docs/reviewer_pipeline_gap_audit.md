...
| RBP/depolymerase evidence | 495 PHROGs/MMseqs receptor-domain rows, 23 Phold/Foldseek receptor-like structural rows, and 105 phage-level module identity signatures for assay phages | Exact module identity signatures improve over RBPbase but do not robustly outperform genome-similarity plus K/O; cold-K support diagnostics show asymmetric fallback behavior; full domain-order architecture remains untested | Expand/curate receptor evidence and test external/generalization cohorts before claiming superiority | Benchmark implemented; module signal available but claim not supported |
